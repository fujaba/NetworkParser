...
public abstract class AbstractListOLD<V> implements BaseItem {
	...
	public AbstractArray<V> subSet(V fromElement, V toElement) {
		AbstractArray<V> newList = getNewInstance();
	 ...
	 *
	 */
	public AbstractArray<V> headSet(V toElement, boolean inclusive) {
		AbstractArray<V> newList = getNewInstance();
	 ...
	 *
	 */
	public AbstractArray<V> tailSet(V fromElement, boolean inclusive) {
		AbstractArray<V> newList = getNewInstance();
	 ...
	 *             number.
	 */
	public AbstractArray<V> put(int index, V key) throws RuntimeException {
		if (index < 0) {
			throw new RuntimeException("EntityList[" + index + "] not found.");
		}
		if (index < size()) {
	...
	public AbstractArray<V> withCopyList(List<V> reference) {
		this.keys = reference;
		return this;
	}
	 ...
	 * @return the List
	 */
	public AbstractArray<V> withInsert(V element, V beforeElement) {
		int index = getIndex(beforeElement);
		add(index, element);
		return this;
	}

	public abstract AbstractArray<V> with(Object... values);

	@SuppressWarnings("unchecked")
	public <ST extends AbstractArray<V>> ST withoutList(Collection<?> values) {
	...
	public AbstractArray<V> clone(AbstractArray<V> newInstance) {
	...
	@SuppressWarnings("unchecked")
	public <ST extends AbstractArray<V>> ST withList(Collection<?> values) {
		for (Iterator<?> i = values.iterator(); i.hasNext();) {
			with(i.next());
		}
		return (ST) this;
	...
	@SuppressWarnings("unchecked")
	public <ST extends AbstractArray<V>> ST getFirstItems(int size) {
	...
	@SuppressWarnings("unchecked")
	public <ST extends AbstractArray<V>> ST getLastItems(int size) {
		AbstractArray<V> newInstance = getNewInstance();
		int count = this.size();
		if (size < 0) {
			size = count;

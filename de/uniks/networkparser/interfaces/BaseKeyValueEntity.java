...
public abstract class BaseKeyValueEntity implements BaseListEntity, FactoryEntity {
	/**
	...
	public BaseKeyValueEntity withValues(Map<?, ?> map) {
		if (map != null) {
	 ...
	 * @return this.
	 */
	public BaseKeyValueEntity addToList(String key, Object value) {
	...
	public BaseKeyValueEntity insert(int pos, String key, Object value){
		EntityUtil.testValidity(value);
		Object object = this.get(key);
		if (object != null) {
			this.put(key, value);
	 ...
	 */
	public static String[] getNames(BaseKeyValueEntity jo) {
		int length = jo.size();
		if (length == 0) {
			return null;
		}
	...
	public String getString(String key) {
		...
		} else if (object instanceof BaseKeyValueEntity) {
	 ...
	 */
	public BaseKeyValueEntity increment(String key) throws RuntimeException{
	...
	@Override
	public abstract BaseListEntity getNewArray();
	 ...
	 *         empty.
	 */
	public BaseListEntity names() {
		BaseListEntity ja = getNewArray();
	 ...
	 * @return this.
	 */
	public BaseKeyValueEntity put(String key, boolean value) {
	 ...
	 * @return this.
	 */
	public BaseKeyValueEntity put(String key, Collection<?> value) {
	 ...
	 * @return this.
	 */
	public BaseKeyValueEntity put(String key, double value) {
	 ...
	 * @return this.
	 */
	public BaseKeyValueEntity put(String key, int value) {
	 ...
	 * @return this.
	 */
	public BaseKeyValueEntity put(String key, long value) {
			...
			if (end == 0) {
				...
				if (id >= 0 || id == -2) {
					if (child instanceof EntityList) {
						if (end == len + 2) {
							// Get List
							BaseListEntity result = getNewArray();
							BaseListEntity items = (BaseListEntity) child;
							for (int z = 0; z < items.size(); z++) {
								result.with(((BaseKeyValueEntity) items.get(z)).getValue(key
										.substring(end + 1)));
							}
							return result;
						}
						...
						if (list.size() >= id) {
							return ((BaseKeyValueEntity) list.get(id)).getValue(key
									.substring(end + 1));
						}
					}
				} else {
					return ((BaseKeyValueEntity) child).getValue(key.substring(end + 1));
				}
			}
		}
		return null;
	...
	public BaseKeyValueEntity setValue(String key, Object value) {
		int len = 0;
		int end = 0;
		int id = 0;
		for (; len < key.length(); len++) {
			...
			if (end == 0) {
				...
				if (id >= 0 || id == -2) {
					if (child instanceof EntityList) {
						...
						if (list.size() >= id) {
							((BaseKeyValueEntity) list.get(id)).setValue(
									key.substring(end + 1), value);
						}
					}
				} else {
					((BaseKeyValueEntity) child).setValue(key.substring(end + 1), value);
				}
			}
		} else {
			put(key.substring(0, len), value);

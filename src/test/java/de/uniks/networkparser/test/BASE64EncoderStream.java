...
/**
 * This class implements a BASE64 encoder. It is implemented as a FilterOutputStream, so one can just wrap this class around any output stream and
 * write bytes into this filter. The encoding is done as the bytes are written out.
 *
 * @author John Mani
 * @author Bill Shannon
 */
...
public class BASE64EncoderStream extends FilterOutputStream
{
    ...
    * @param bytesPerLine number of bytes per line. The encoder inserts a CRLF sequence after the specified number of bytes, unless bytesPerLine is
    *        Integer.MAX_VALUE, in which case no CRLF is inserted. bytesPerLine is rounded down to a multiple of 4.
    */
   ...
   /**
    * Encodes <code>len</code> bytes from the specified <code>byte</code> array starting at offset <code>off</code> to this output stream.
    *
    * @param b the data.
   ...
   /**
    * Encode the data stored in <code>buffer</code>. Uses <code>outbuf</code> to store the encoded data before writing.
    *
    * @exception IOException if an I/O error occurs.
    */
   ...
   /**
    * Base64 encode a byte array. No line breaks are inserted. This method is suitable for short strings, such as those in the IMAP AUTHENTICATE
   ...
   /**
    * Internal use only version of encode. Allow specifying which part of the input buffer to encode. If outbuf is non-null, it's used as is.
    * Otherwise, a new output buffer is allocated.
    * @param inbuf Input Buffer
    * @param off Offset of Buffer
    * @param size Size of Buffer
    * @param outbuf OutputBuffer
    * @return outbuf
    */
   ...
   /***
    * begin TEST program public static void main(String argv[]) throws Exception { FileInputStream infile = new FileInputStream(argv[0]);
    * BASE64EncoderStream encoder = new BASE64EncoderStream(System.out); int c;
    *
    * while ((c = infile.read()) != -1) encoder.write(c); encoder.close(); } end TEST program
    **/
}

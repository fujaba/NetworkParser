	...
	protected boolean callBack(GraphMember member, boolean value, String... params) {
		if(member instanceof Clazz) {
			...
			if(params != null && params.length>1) {
				Annotation anno = Annotation.create("XmlElementWrapper", "name", params[0]).withImport("javax.xml.bind.annotation.XmlElementWrapper").withScope("getter");
				anno.withNext(Annotation.create("XmlElement", "name", params[1]).withImport("javax.xml.bind.annotation.XmlElement").withScope("getter"));
				attribute.with(anno);
			}else if(value) {

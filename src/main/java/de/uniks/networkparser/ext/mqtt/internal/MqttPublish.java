...
public class MqttPublish extends MqttWireMessage {
	...
	// Methods from MqttPersistableWireMessage
	public int getHeaderOffset() {
		return 0;
	}
	public byte[] getHeaderBytes() throws MqttException {
		try {
			return getHeader();
		}
		catch (MqttException ex) {
			throw MqttException.withReason(MqttException.REASON_CODE_DEFAULT, ex.getCause());
		}
	}
	
	public int getHeaderLength() throws MqttException {
		return getHeaderBytes().length;
	}

	public byte[] getPayloadBytes() throws MqttException {
		try {
			return getPayload();
		}
		catch (MqttException ex) {

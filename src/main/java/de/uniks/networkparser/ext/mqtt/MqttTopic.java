...
public class MqttTopic {
	...
	public static void validate(String topicString, boolean wildcardAllowed)
		...
		if (wildcardAllowed) {
			// Only # or +
			if (EntityUtil.equalsAny(topicString, new String[] { MULTI_LEVEL_WILDCARD, SINGLE_LEVEL_WILDCARD })) {
				return;
			}
			...
			// the topic tree
			if (EntityUtil.countMatches(topicString, MULTI_LEVEL_WILDCARD) > 1
		...
		// *******************************************************************************
		if (EntityUtil.containsAny(topicString, TOPIC_WILDCARDS)) {
			throw new IllegalArgumentException(

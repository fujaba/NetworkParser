...
public class ReflectionBackBoxTester {
	public static final String NULLVALUE="nullValue";
	public static final String MINVALUE="minValue";
	public static final String MAXVALUE="maxValue";
	public static final String RANDOMVALUE="randomValue";
	private SimpleSet<String> tests=new SimpleSet<String>().with(NULLVALUE,MINVALUE);
	private SimpleSet<String> ignoreMethods=new SimpleSet<String>().with("wait", "notify", "notifyAll");
	private SimpleSet<String> ignoreClazz=new SimpleSet<String>().with("de.uniks.networkparser.NetworkParserLog");
	
	
	
	public ReflectionBackBoxTester withIgnoreMethods(String... values) {
		if(values == null) {
			return this;
		}
		for(String item : values) {
			this.ignoreMethods.add(item);
		}
		return this;
	}
	
	public ReflectionBackBoxTester withIgnoreClazzes(String... values) {
		if(values == null) {
			return this;
		}
		for(String item : values) {
	...
	public void test(String packageName, NetworkParserLog logger) throws ClassNotFoundException, IOException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		...
		for(Class<?> clazz : classesForPackage) {
			...
			if(this.ignoreClazz.contains(clazz.getName())) {
				item.append("ignore");
				continue;
			}

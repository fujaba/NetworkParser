 ...
 * <p>
 * Illustrative code for the AES block cipher (Rijndael). Handles a single block
 * encryption or decryption, with diagnostic logging of intermediate values if
 * required.
 * <p>
 * AES is a block cipher with a key length of 16/24/32 bytes and a block length
 * 16 bytes.
 * <p>
 * AES (Rijndael) was designed by Joan Daemen and Vincent Rijmen, and was
 * accepted as the US NIST's Advanced Encryption Standard in 2000.
 * <p>
...
public class AES {
	...
	static {
		int i, j;
		// produce log and alog tables, needed for multiplying in the field
		// GF(2^8)
		alog[0] = 1;
		for (i = 1; i < 256; i++) {
	...
	public AES withKey(byte[] key) {
		...
		// allocate arrays to hold en/decrypt session keys (by byte rather than
		// word)
		Ke = new byte[numRounds + 1][BLOCK_SIZE]; // encryption round keys
		Kd = new byte[numRounds + 1][BLOCK_SIZE]; // decryption round keys

		// copy key into start of session array (by word, each byte in own
		// array)
		for (i = 0, j = 0; i < Nk; i++) {
			w0[i] = key[j++];
			w1[i] = key[j++];
		...
		for (i = Nk; i < ROUND_KEY_COUNT; i++) {
			...
			if (i % Nk == 0) {
				...
				t0 = (byte) (S[t1 & 0xFF] ^ rcon[i / Nk]); // nb. constant XOR
															// 1st byte only
				t1 = (byte) (S[t2 & 0xFF]);
				t2 = (byte) (S[t3 & 0xFF]); // nb. RotWord done by reordering
											// bytes used
				t3 = (byte) (S[old0 & 0xFF]);
			} else if ((Nk > 6) && (i % Nk == 4)) {
				// temp = SubWord(temp)
		...
		// now copy values into en/decrypt session arrays by round & byte in
		// round

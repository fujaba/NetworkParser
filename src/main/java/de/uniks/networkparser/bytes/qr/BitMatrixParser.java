...
public final class BitMatrixParser {
   ...
   * </p>
   *
   * @return {@link Version} encapsulating the QR Code is version the valid encoding of version
   *         information
   */
  Version readVersion() {
   ...
   * order to reconstruct the codewords bytes contained within the QR Code.
   * </p>
   *
  ...
  byte[] readCodewords() {
    ...
    /*
     * Get the data mask for the format used in this QR Code. This will exclude some bits from reading
     * as we wind through the bit matrix.
     */
    int dimension = bitMatrix.getHeight();
    ...
    for (int j = dimension - 1; j > 0; j -= 2) {
      if (j == 6) {
        /*
         * Skip whole column with vertical alignment pattern; saves time and makes the other code proceed
         * more cleanly
         */
        j--;
      }
  ...
  /**
   * <p>
   * Implementations of this method reverse the data masking process applied to a QR Code and make its
   * bits ready to read.
   * </p>
   *
   ...
   * {@link #readFormatInformation()} and the {@link #readVersion()}. Before proceeding with
   * {@link #readCodewords()} the {@link #mirror()} method should be called.
   *

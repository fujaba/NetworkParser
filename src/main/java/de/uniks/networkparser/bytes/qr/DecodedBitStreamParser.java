...
final class DecodedBitStreamParser {
    ...
    do {
      ...
      if (mode != Mode.TERMINATOR) {
        if (mode == Mode.FNC1_FIRST_POSITION || mode == Mode.FNC1_SECOND_POSITION) {
          ...
          /*
           * sequence number and parity is added later to the result metadata Read next 8 bits (symbol
           * sequence #) and 8 bits (parity data), then continue
           */
          ...
          if (mode == Mode.HANZI) {
            ...
            if (mode == Mode.NUMERIC) {
              ...
              if (!(decodeByteSegment(bits, result, count, byteSegments))) {
                return null;
              }
  ...
  private static boolean decodeHanziSegment(BitArray bits, CharacterBuffer result, int count) {
    ...
    /*
     * Each character will require 2 bytes. Read the characters as 2-byte pairs and decode as GB2312
     * afterwards
     */
  ...
  private static boolean decodeKanjiSegment(BitArray bits, CharacterBuffer result, int count) {
    ...
    /*
     * Each character will require 2 bytes. Read the characters as 2-byte pairs and decode as Shift_JIS
     * afterwards
     */
    byte[] buffer = new byte[2 * count];
    int offset = 0;

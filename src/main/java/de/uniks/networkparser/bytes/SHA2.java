...
public class SHA2 {
   ...
   *
   * Optionally, length of the data can be specified to hash fewer bytes than data.length.
   *
   * Throws error when trying to update already finalized hash: instance must be reset to use it
   * again.
   * 
  ...
  public SHA2 finish(byte[] out) {
    if (!this.finished) {
      int hashed = this.bytesHashed;
      int left = this.bufferLength;
      int bitLenHi = (hashed / 0x20000000) | 0;
      int bitLenLo = hashed << 3;
      int padLength = (hashed % 64 < 56) ? 64 : 128;

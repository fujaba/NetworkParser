...
public class JavaSetAttribute extends Template {
	public JavaSetAttribute() {
		...
		this.withTemplate("{{#if {{#NOT}}{{modifiers#contains(static)}}{{#ENDNOT}}}}" + "{{#import {{type(false)}}}}"
				...
				"		{{#listType}} result = new {{#listType}}();", "{{#if {{#feature PATTERN}}}}",
				"		if(listener != null) {", "{{#import " + SimpleEvent.class.getName() + "}}",
				...
				"				result.add(obj.{{namegetter}}());", "			}", "		} else {",
				"			for ({{file.member.name}} obj : this) {",
				"				{{type}} item = obj.{{namegetter}}();",
				"				for(int i=0;i<filter.length;i++) {",
				"{{#if {{#or}}{{type}}==int {{type}}==boolean{{#endor}}}}",
				"					if (filter[i] == item) {" + "{{#else}}",
				"					if (filter[i].equals(item)) {" + "{{#endif}}",
				"						result.add(item);", "						break;", "					}",
				"				}", "			}", "		}", "		return result;", "	}", "",
				...
				"		{{SetName}} result = new {{SetName}}();", "		for ({{name}} obj : this) {",
				...
				"				result.add(obj);", "			}", "		}", "		return result;", "	}", "", "",
				"{{#endif}}" + "	public {{file.member.name}}Set with{{Name}}({{type}} value) {",
				"		for ({{file.member.name}} obj : this) {", "			obj.set{{Name}}(value);", "		}",
				"		return this;", "	}", "" + "{{#endif}}");
	}
}

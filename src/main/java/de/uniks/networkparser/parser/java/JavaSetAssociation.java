...
public class JavaSetAssociation extends Template {
	public JavaSetAssociation() {
		...
		this.withTemplate("{{#template VALUE}}", "{{#if {{other.isImplements}}==false}}",
				...
				"		for ({{file.member.name}} obj : this) {", "			obj.with{{other.Name}}(value);", "		}",
				"		return this;", "	}", "",

				"{{#import {{other.clazz.fullName}}}}" + "{{#if {{other.cardinality}}==MANY}}",
				"	public {{file.member.name}}Set without{{other.Name}}({{other.clazz.name}} value) {",
				"		for ({{file.member.name}} obj : this) {", "			obj.without{{other.Name}}(value);",

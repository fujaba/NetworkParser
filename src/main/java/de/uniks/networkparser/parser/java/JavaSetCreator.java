...
// SendableCreator
				...
				"{{#if {{#and}}{{item.className}}==" + Association.class.getName()
						+ " {{item.other.isImplements}}==false{{#endand}}}}",
				"{{#import {{item.other.clazz.fullName}}}}", "		{{name}}.PROPERTY_{{item.other.NAME}},",
				"{{#endif}}", "{{#endfor}}", "{{#if {{#feature DYNAMICVALUES}}}}", "	SendableEntityCreator.DYNAMIC",
				"{{#endif}}", "	};", "",
				...
				"			return element.get{{item.other.Name}}();", "		}", "", "{{#endif}}", "{{#endfor}}",
